...
function Lock(iab, ibase) {
    if (
        !(
            iab instanceof Int32Array &&
            ibase | (0 == ibase) &&
            ibase >= 0 &&
            ibase + Lock.NUMINTS <= iab.length
        )
    )
        throw new Error(
            "Bad arguments to Lock constructor: " + iab + " " + ibase
        );
    this.iab = iab;
    this.ibase = ibase;
}
...
//
// Returns 'ibase'.
Lock.initialize = function(iab, ibase) {
    if (
        !(
            iab instanceof Int32Array &&
            ibase | (0 == ibase) &&
        ...
        throw new Error(
            "Bad arguments to Lock initializer: " + iab + " " + ibase
        );
...
function Cond(lock, ibase) {
    if (
        !(
            ...
            ibase | (0 == ibase) &&
            ibase >= 0 &&
            ibase + Cond.NUMINTS <= lock.iab.length
        )
    )
        throw new Error(
            "Bad arguments to Cond constructor: " + lock + " " + ibase
        );
    this.iab = lock.iab;
    this.ibase = ibase;
    this.lock = lock;
...
//
// Returns 'ibase'.
Cond.initialize = function(iab, ibase) {
    if (
        !(
            iab instanceof Int32Array &&
            ibase | (0 == ibase) &&
            ibase >= 0 &&
            ibase + Cond.NUMINTS <= iab.length
        )
    )
        throw new Error(
            "Bad arguments to Cond initializer: " + iab + " " + ibase
        );
    Atomics.store(iab, ibase, 0);
    return ibase;
};
